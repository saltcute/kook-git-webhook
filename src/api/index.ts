...
export async function addWebhooks(
    ...
    webhooks.on("ping", async ({ id, name, payload }) => {
        ...
        if (payload.repository) {
            card.addTitle("喵喵！").addDivider()
            card.addText(
                `收到了来自 \`${payload.repository.full_name}\` 的 ping 事件喵！`
            )
    ...
    webhooks.on("push", async ({ id, name, payload }) => {
        ...
        if (payload.before.startsWith("0000000")) {
            // Creating a new branch
            card.addContext(
                `创建${getBranchTypeString()} [${getBranchName()}](${getBranchURL()})`
            )
        } else if (payload.after.startsWith("0000000")) {
            // Deleting a branch
            card.addContext(
                `删除${getBranchTypeString()} [${getBranchName()}](${getBranchURL()})`
            )
        } else {
            card.addContext(
                `${payload.head_commit ? `${new Date(payload.head_commit?.timestamp).toLocaleString("zh-CN").replaceAll(":", "\\:")} 时` : ""}对 [${getBranchName()}](${getBranchURL()}) 的 ${payload.commits.length} 个 commit`
            )
        }
        ...
        if (payload.commits.length > 0) {
            card.addContext(
                `正在显示 ${payload.commits.length > 20 ? 20 : payload.commits.length} 共 ${payload.commits.length} 个 commit | [查看总共 ${payload.commits.length} 个变动](${payload.compare})`
            )
            ...
            for (let i = 0; i < payload.commits.length && i < 20; i++) {
                ...
                card.addContext(
                    `[${lastCommit.substring(0, 7)} -> ${commit.id.substring(0, 7)}](https://github.com/${payload.repository.full_name}/compare/${lastCommit.substring(0, 7)}...${commit.id.substring(0, 7)})`
...
`)
        ...
        const { err } = await client.API.message.create(
            MessageType.CardMessage,
            channelId,
            card
        )
